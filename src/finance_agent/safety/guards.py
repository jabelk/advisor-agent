...
def get_kill_switch(conn: sqlite3.Connection) -> bool:
    ...
    row = conn.execute("SELECT value FROM safety_state WHERE key = 'kill_switch'").fetchone()
    if not row:
        return False
...
def get_risk_settings(conn: sqlite3.Connection) -> dict[str, float | int]:
    ...
    row = conn.execute("SELECT value FROM safety_state WHERE key = 'risk_settings'").fetchone()
    if not row:
...
def update_risk_setting(
    ...
    if value < min_val or value > max_val:
        raise ValueError(f"Value {value} for '{key}' is out of range [{min_val}, {max_val}]")
