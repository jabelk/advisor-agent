...
def translate_filters_to_report(
    ...
    if filters.search is not None:
        ...
        warnings.append(
            "search filter partially supported in Reports (per-column contains on name fields)"
        )
    ...
    if sort_non_default:
        warnings.append(
            "sort_by/sort_dir not supported in Reports (Salesforce applies default sort)"
        )

    if filters.limit != 50:
        warnings.append("limit not supported in Reports (all matching contacts shown)")
...
def ensure_report_folder(sf: Salesforce) -> str:
    ...
    result = sf.query("SELECT Id, Name FROM Folder WHERE Name = 'Client Lists' AND Type = 'Report'")
...
def delete_report(sf: Salesforce, name: str) -> bool:
    ...
    result = sf.query(f"SELECT Id, Name FROM Report WHERE Description LIKE '%{ADVISOR_AGENT_TAG}%'")
