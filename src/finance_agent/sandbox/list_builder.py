"""Saved list CRUD and natural language query translation (020-client-list-builder)."""
...
def _get_data_dir(data_dir: Path | None = None) -> Path:
    ...
    if data_dir is not None:
        ...
        base = os.environ.get("ADVISOR_AGENT_DATA_DIR", str(Path.home() / ".advisor-agent"))
        data_dir = Path(base)
    data_dir.mkdir(parents=True, exist_ok=True)
    return data_dir


def _lists_path(data_dir: Path) -> Path:
    return data_dir / "saved_lists.json"


def _load_lists(data_dir: Path) -> dict:
...
def _save_lists(data_dir: Path, data: dict) -> None:
    path = _lists_path(data_dir)
...
def save_list(
    ...
    """Save a named list. Raises ValueError if name exists (case-insensitive)."""
    d = _get_data_dir(data_dir)
    data = _load_lists(d)

    # Case-insensitive duplicate check
    for existing_name in data["lists"]:
    ...
    data["lists"][name] = saved.model_dump()
    _save_lists(d, data)
    return saved


def get_saved_lists(data_dir: Path | None = None) -> list[SavedList]:
    """Return all saved lists sorted by name."""
    d = _get_data_dir(data_dir)
    data = _load_lists(d)
    lists = [SavedList(**v) for v in data["lists"].values()]
    return sorted(lists, key=lambda x: x.name.lower())


def get_saved_list(name: str, data_dir: Path | None = None) -> SavedList | None:
    """Find a saved list by name (case-insensitive). Returns None if not found."""
    d = _get_data_dir(data_dir)
    data = _load_lists(d)
    for key, val in data["lists"].items():
        if key.lower() == name.lower():
            return SavedList(**val)
    return None


def run_saved_list(sf, name: str, data_dir: Path | None = None) -> dict:
    """Execute a saved list's filters against Salesforce. Raises ValueError if not found."""
    from finance_agent.sandbox.storage import format_query_results, list_clients

    d = _get_data_dir(data_dir)
    data = _load_lists(d)

    # Find the list (case-insensitive)
    found_key = None
    ...
    saved = SavedList(**data["lists"][found_key])
    f = saved.filters
    ...
    # Update last_run_at
    now = datetime.now(timezone.utc).isoformat()
    data["lists"][found_key]["last_run_at"] = now
    _save_lists(d, data)

    return format_query_results(clients, f)
...
def update_saved_list(
    ...
    """Update a saved list. Raises ValueError if not found or rename conflicts."""
    d = _get_data_dir(data_dir)
    data = _load_lists(d)
    ...
    entry = data["lists"][found_key]

    # Handle rename
    new_name = updates.get("name")
    if new_name and new_name.lower() != found_key.lower():
        for key in data["lists"]:
            if key.lower() == new_name.lower():
                raise ValueError(f"List '{new_name}' already exists")
        # Remove old key, will re-add with new name
        del data["lists"][found_key]
        entry["name"] = new_name
        found_key = new_name
    ...
    if "filters" in updates:
        f = updates["filters"]
        entry["filters"] = f.model_dump() if hasattr(f, "model_dump") else f

    data["lists"][found_key] = entry
    _save_lists(d, data)
    return SavedList(**entry)


def delete_saved_list(name: str, data_dir: Path | None = None) -> bool:
    """Delete a saved list. Returns True if deleted, False if not found."""
    d = _get_data_dir(data_dir)
    data = _load_lists(d)
